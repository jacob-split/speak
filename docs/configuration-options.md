...
| `SPEAK_CALL_LOG_DIR` / `CALL_LOG_DIR` | backend/scripts env | `call-logs` | Directory containing persisted call-event JSONL logs. `CALL_LOG_DIR` is a script-side compatibility alias used by CallTools proof and recording audits. Useful for isolated fixtures and tests. |
...
| `SPEAK_HERE_NOW_PROXY_ORIGINS` | backend env | empty | Optional comma-separated origins allowed to use the `/api/here-now/*` here.now proxy. `SPEAK_FILE_WAREHOUSE_ORIGINS` is still accepted as a legacy deployment alias. |

## Provider And Delivery Runtime

| Option | Scope | Default | Purpose |
| --- | --- | --- | --- |
| `HUME_API_KEY` | backend env/keychain | required for live voice/provider sync | Private voice provider API key. On macOS development hosts, `server/secrets.mjs` also checks Keychain service `speak-hume-api-key` through the same secret boundary used by Inworld. |
...
| `XAI_API_KEY` | backend env/keychain | required for xAI live calls/options | Private xAI key. macOS development also reads Keychain service `Speak Production`, account `XAI_API_KEY`; it is never exposed to the browser. |
...
| `PERSONAL_PHONE_SPEAK_HANDOFF_SECRET` | backend env/keychain | Keychain service `personal-phone-speak-handoff-secret`; required in production env | Shared bearer secret used only by the Personal Phone Worker handoff and resolve routes. The macOS mirror is also stored under service `Speak Production`, account `PERSONAL_PHONE_SPEAK_HANDOFF_SECRET`; raw values are never returned. |
...
| `CALLTOOLS_BASE_URL` | backend env | `https://east-2.calltools.io/api` | CallTools account silo API base. |
...
| `GET /api/calltools/readiness` | backend read API | n/a | Read-only Phone-as-Agent preflight for API auth, agent user, WebRTC phone, Speak media gateway, campaign/caller ID/source health, dispositions, live calls, outcome writeback, and sanitized watchdog status. `runtimeReady` covers transport dependencies. `directStartReady` is retained as legacy diagnostic output only and never authorizes `/api/calls/start`. `campaignReady` proves the campaign/session established by Go available: native AgentStatus readiness with `webPhoneRegisteredOn`, selected-campaign binding, `/campaignagents/{app_user_id}` readiness, active/originating campaign state, caller ID, source inventory, and aggregate agent counts. `dutyMonitor` reports the server-owned Available lease and frozen binding; the backend retains and repairs Available until explicit Go unavailable. |
...
| `CALLTOOLS_PROOF_REQUIRE_LATENCY` / `CALLTOOLS_PROOF_REQUIRE_AUDIO_QUALITY` | script env | `true` / `true` | Diagnostic latency/audio thresholds used by retained log readers and legacy direct-proof scripts. They remain useful evidence but do not by themselves certify a native campaign invite. |
...
| `CALLTOOLS_PROOF_MAX_FIRST_USER_TO_ASSISTANT_MESSAGE_MS` | script env | fallback for audio gate | Backward-compatible fallback for older environments. Assistant transcript-after-caller is still required separately, but text event timing is not the primary real-time audio responsiveness gate. |
...
| `CALLTOOLS_PROOF_MAX_LEAD_AUDIO_TO_USER_MESSAGE_MS` / `CALLTOOLS_PROOF_MAX_DIAL_TO_GATEWAY_ATTACH_MS` | script env | disabled unless set | Optional diagnostic latency gates for the legacy-named `firstCallToolsLeadAudioToFirstUserMessage` caller-audio-to-user-message timing and CallTools-dial-to-gateway-attach timing. Keep unset unless measured production evidence proves a safe threshold. |
...
| `CALLTOOLS_FULL_AUDIT_RECORDING_TRANSCRIPT_LOG` / `SPEAK_CALLTOOLS_PROOF_LOG` / `--calltoolsRecordingLog=<file>` / `--log=<file>` | recording audit env/flag | recent operational logs | Optional call-log override for CallTools recording-derived transcript review. Direct `audit:calltools-recording-transcript` runs with only `--callControlId` scan recent operational logs before falling back to the current day, so native campaign calls remain discoverable after UTC midnight. Legacy full-audit proof artifacts may still carry a `transcriptParity.logFile`, but they do not certify campaign following. |
...
| `SPEAK_LINK_URL` / `PORTAL_URL` | backend env | empty | Optional configurable customer/workspace link used only by the legacy confirmed link-delivery alias. Generic `send_text_message` and `send_email` always use explicit content from the active agent instructions and do not depend on this value. `/api/health` reports `delivery.linkConfigured`; `PORTAL_URL` is the older runtime-config alias. |
| `WORKSPACE_EMAIL_ACCOUNT` | backend env | empty | Sender/readback email account for workspace email sends and replies. |
| `WORKSPACE_EMAIL_READ_GOG_ACCOUNT` | backend/script env | falls back to `WORKSPACE_EMAIL_GOG_ACCOUNT` | GOG auth account used for bounded Workspace/Gmail source sync. The visible mailbox identity remains `WORKSPACE_EMAIL_ACCOUNT`. |
| `WORKSPACE_EMAIL_SEND_GOG_ACCOUNT` | backend/script env | falls back to `WORKSPACE_EMAIL_GOG_ACCOUNT` | GOG auth account used for operator-confirmed email replies. The GOG auth store is additive, so authorizing Operator, Operator, Apps, Teammate, Reports, Hello, or another mailbox must not replace existing stored accounts; set this variable to choose the mailbox Speak sends through for this deployment. If it differs from `WORKSPACE_EMAIL_ACCOUNT`, reply readiness also requires an accepted send-as alias for `WORKSPACE_EMAIL_ACCOUNT`; managing that alias requires Gmail settings scope such as `gmail.settings.basic`, `gmail.settings.sharing`, or full mail scope. |
| `WORKSPACE_EMAIL_GOG_ACCOUNT` | backend/script env | `WORKSPACE_EMAIL_ACCOUNT` | Legacy/shared GOG auth-account fallback used when read/send-specific accounts are not set. |
...
| `GMAIL_READ_GOG_ACCOUNT` | backend/script env | empty | Backward-compatible alias for `WORKSPACE_EMAIL_READ_GOG_ACCOUNT`. |
| `GMAIL_SEND_GOG_ACCOUNT` | backend/script env | empty | Backward-compatible alias for `WORKSPACE_EMAIL_SEND_GOG_ACCOUNT`. |
| `GMAIL_GOG_ACCOUNT` | backend/script env | empty | Backward-compatible shared GOG auth-account alias. |
...
| `GOG_WRAPPER` | backend env | `/usr/local/bin/split-speak-gog` | Surface-native workspace email gog wrapper. It fails closed until a valid Gmail OAuth account is stored for `speak-target`. |
| `HOME` | CallTools gateway systemd env | `/opt/speak` | Surface runtime home for the `speak-target` gateway process. |
| `PLAYWRIGHT_BROWSERS_PATH` | CallTools gateway systemd env | `/opt/speak-browsers` | Surface-owned Playwright browser bundle used by the persistent CallTools gateway. |
...
| `SPEAK_CODEX_DEBUG_TOKEN` | backend env | empty | Backward-compatible token alias. |
...
| `SPEAK_CODEX_DEBUG_ALLOWED_EMAILS` | backend env | empty | Backward-compatible allowed-email alias. |
| `SPEAK_SMART_CONFIG_ALLOWED_DOMAINS` | backend env | empty | OAuth/session allowed domains. |
| `SPEAK_CODEX_DEBUG_ALLOWED_DOMAINS` | backend env | empty | Backward-compatible allowed-domain alias. |
...
| `GOOGLE_OAUTH_CLIENT_ID` | backend env | alias | Google OAuth client ID alias. |
| `GOOGLE_CLIENT_ID` | backend env | alias | Google OAuth client ID alias. |
| `SPEAK_GOOGLE_OAUTH_CLIENT_SECRET` | backend env | alias chain | Google OAuth client secret. |
| `GOOGLE_OAUTH_CLIENT_SECRET` | backend env | alias | Google OAuth client secret alias. |
| `GOOGLE_CLIENT_SECRET` | backend env | alias | Google OAuth client secret alias. |
...
| `SPEAK_SMART_CONFIG_CODEX_REASONING` | backend env | alias | Smart Config reasoning alias. |
| `SPEAK_SMART_CONFIG_CODEX_SERVICE_TIER` | backend env | `priority` | Smart Config service tier. |
| `SPEAK_SMART_CONFIG_CODEX_SPEED` | backend env | alias | Smart Config service-tier alias. |
...
| Option | Scope | Default | Purpose |
| --- | --- | --- | --- |
| `SPEAK_QA_BASE_URL` / `REAL_QA_BASE_URL` | QA env | `http://127.0.0.1:5173/speak` | Browser QA target. `REAL_QA_BASE_URL` is the compatibility alias still read by `qa:ui-contract`. |
...
| `SPEAK_FULL_AUDIT_REMOTE_CWD` / `--remoteCwd=<path>` | full audit env/flag | `/opt/speak` | Surface live-runtime path used only when `--sshHost` explicitly runs production checks remotely. Prefer `--production` when running on Omarchy itself. |
...
| `SPEAK_FULL_AUDIT_LIVE_PROOF_DIR` / `--liveProofDir=<path>` | full audit env/flag | temp directory | Directory where `qa:full-audit --include-live` writes the generated CallTools proof JSON before feeding it into S-tier validation. |
| `SPEAK_PRODUCT_PAGE_URL` | docs/brand QA env | `https://files.split-llc.com/speak/` | Published product/docs URL fetched by `qa:brand-marketing` for hosted HTML, raw Markdown, internal link, and asset byte parity. |
| `SPEAK_BRAND_FETCH_TIMEOUT_MS` | docs/brand QA env | `12000` | Per-request bound for published docs, link, image, and production-app asset reads. The verifier clamps this to 1-60 seconds so an unhealthy public edge fails with explicit evidence instead of freezing a release indefinitely. |
...
| `codexReasoningEffort` | `none`, `minimal`, `low`, `medium`, `high`, or `xhigh` | `none` | Codex reasoning effort for Codex-auth selections. Missing, blank, and legacy values normalize to `none`; an explicit supported nonzero value remains available. Hume forwards the normalized selection to the Speak CLM bridge. Inworld fixes the effective value to `NONE` while shared tools are active because its chat-completions route rejects function tools combined with nonzero effort. |
...
| `codexFastMode` | boolean | `true` | Provider-aware low-latency path for every Codex-auth selection in both runtimes. Missing or legacy blank values normalize to Fast mode on; an explicit saved `false` remains available. Hume requests the Codex priority service tier. Inworld disables conversational TTS context and uses `fast_start` for earlier first audio; enabling conversational context turns Fast mode off. |
...
| `telnyxConnectionId` | string | backend env readback | Read-only profile compatibility field. Live outbound calls always use workspace `TELNYX_CONNECTION_ID`; saved values never override it. |
| `phoneConnectionId` | string | alias | Neutral compatibility alias; it cannot override workspace `TELNYX_CONNECTION_ID`. |
